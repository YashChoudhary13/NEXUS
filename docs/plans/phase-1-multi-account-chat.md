...
- Target the **same workspace** through the dedicated `sessions:continueWithAgent` RPC,
  classified `REMOTE_ELIGIBLE`. It validates the exact target, flushes durable parent state,
  persists both link directions before renderer notification, and rolls back partial children.
...
- Handoff package v0: use a dedicated transfer-summary prompt plus the existing hidden
  one-shot injection path; content = current objective + completed work + recent decisions and
  rationale + remaining tasks + touched files + commands/tests/results + blockers + git/worktree
  state (from the existing conversation-summary machinery), rendered also as a **visible** system
...
- Parent/child linkage: durable session metadata records `continuedFromSessionId` and an ordered,
  repeatable `continuedToSessionIds` list. Native "continued as →" / "continued from ←" header
  buttons navigate in both directions; repeated continuations retain earlier children.
- UI: session menu action + a small account/model picker dialog (reuses PR-1D components).

**Tests:** `test:linked-handoff` covers server creation/rollback/deletion/repeat behavior,
protocol persistence, renderer metadata, and hidden one-shot injection (**24 pass / 1,660
assertions**); the PR-1D picker dependency passes **37 / 46**; the sessions atom suite passes
**10 / 39** including idempotent child hydration. Manual built-app smoke generated a handoff
through a disposable localhost OpenAI-compatible stub, navigated automatically to the child,
showed the summary, followed both link directions, and created two children from one parent.
...
- [ ] Rename one account, restart, and confirm names, identities, authentication state, and the
      original locked sessions return unchanged.
- [ ] Confirm a same-family duplicate login warns without blocking the connection; confirm two
      different principals and cross-provider accounts do not warn.
- [ ] Start new sessions from every intended provider/account/model combination in both desktop
      and compact pickers; confirm unauthenticated accounts are excluded.
- [ ] From a non-empty parent task, choose **Continue with another agent**, select a different
      account/model, inspect the visible handoff summary, and send the first child message without
      re-explaining context. Confirm parent runtime/transcript/model remain unchanged.
- [ ] Follow child → parent and parent → child header links, create a second continuation from the
      same parent, restart, and verify all links and summaries remain durable.
- [ ] Exercise zero-account, unavailable target, failed-summary/model, and delete parent/child
      paths; confirm no orphaned links, leaked child, or credential crossing.
- [ ] Resolve or explicitly redefine the independently billed subscription acceptance criterion:
      S1 proved distinct ChatGPT user principals routed through one selected runtime workspace.
